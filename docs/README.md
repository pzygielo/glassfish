...
2. If you just want to update the website contents, run the following:

```
mvn -pl website install
```

3. Use the `local` Maven profile which automates the Jekyll setup and build process:

   **Build the Jekyll site:**
   ```bash
   mvn -f publish/pom.xml compile -Plocal
   ```

   **Serve the site locally:**
   ```bash
   mvn -f publish/pom.xml exec:exec@serve-site -Plocal
   ```

The site will be available at http://localhost:8123

**Note:** If `bundle` or `jwebserver` are not on your PATH, you can specify their locations using `-Dbundle.executable=/path/to/bundle` and `-Djwebserver.executable=/path/to/jwebserver` properties.
